...
- **Explicit HTTP surface**: `apis:` lists the APIs served over HTTP. It gates route
  ...
  depend on them. Omitting the key serves everything the providers supply; an empty list
  serves nothing. The stack administration APIs (`admin`, `inspect`, `providers`,
  `prompts`) are always served.
