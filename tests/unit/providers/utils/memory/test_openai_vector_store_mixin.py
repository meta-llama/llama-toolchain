...
class TestMetadataStoreEnforcement:
    ...
    async def test_delete_vector_store_file_decrements_usage_bytes(self):
        """Deleting a file must give back its share of the store's usage_bytes (#6542 follow-up)."""
        store_info = _make_full_store_info("vs_allowed", "allowed")
        store_info["file_ids"] = ["file_1"]
        store_info["usage_bytes"] = 150
        store_info["file_counts"] = {
            "total": 1,
            "completed": 1,
            "cancelled": 0,
        ...
        assert mixin.openai_vector_stores["vs_allowed"]["usage_bytes"] == 50

    async def test_delete_vector_store_file_clamps_usage_bytes_at_zero(self):
        """A stale/undercounted store total must not go negative on delete."""
        store_info = _make_full_store_info("vs_allowed", "allowed")
        store_info["file_ids"] = ["file_1"]
        store_info["usage_bytes"] = 20  # less than the file's own usage_bytes
        store_info["file_counts"] = {
            "total": 1,
            "completed": 1,
            "cancelled": 0,
        ...
        assert mixin.openai_vector_stores["vs_allowed"]["usage_bytes"] == 0
